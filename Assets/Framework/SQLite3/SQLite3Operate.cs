    ...
    public class SQLite3Operate
    {
        ...
        public bool ValueExists(string InTableName, string InProperty, object InValue)
        {
            ...
            stringBuilder.Append("SELECT * FROM ")
                         ...
                         .Append(InProperty)
                         .Append(" = ")
                         .Append(Utility.ConvertValueType(InValue));
        ...
        /// <returns></returns>
        public bool ValueExists<T>(T InObject) where T : SyncBase
        {
            if (0 != (SBLength = stringBuilder.Length)) stringBuilder.Remove(0, SBLength);
        ...
        public bool Insert(string InTableName, params object[] InData)
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                stringBuilder.Append(Utility.ConvertValueType(InData[i])).Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
            stringBuilder.Append(")");
        ...
        public bool InsertT<T>(T InValue) where T : SyncBase
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                stringBuilder.Append(Utility.ConvertValueType(info.GetValue(InValue, null), info.PropertyType)).Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
            stringBuilder.Append(")");
            ...
            if (count > 0)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    for (int j = 0; j < length; j++)
                    {
                        ...
                        stringBuilder.Append(Utility.ConvertValueType(info.GetValue(InValue[i], null), info.PropertyType))
                            .Append(", ");
                    }
                    stringBuilder.Remove(stringBuilder.Length - 2, 2);
                    stringBuilder.Append(")");
        ...
        {
            return Update(InTableName, InUpdateFieldName, InUpdateValue, InConditionFieldName + " = " + Utility.ConvertValueType(InConditionValue));
        }
        ...
        public bool Update(string InTableName, string InUpdateFieldName, object InUpdateValue, string InCondition)
        {
            ...
            stringBuilder.Append("UPDATE ").Append(InTableName)
                .Append(" SET ").Append(InUpdateFieldName).Append(" = ").Append(Utility.ConvertValueType(InUpdateValue))
        ...
        public bool UpdateMultiValue(string InTableName, string[] InFieldName, object[] InValues, string InCondition)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                stringBuilder.Append(InFieldName[i])
                    .Append(" = ")
                    .Append(Utility.ConvertValueType(InValues[i]))
                    .Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
        ...
        public bool UpdateT<T>(T InValue) where T : SyncBase
        {
            ...
            for (int i = 1; i < length; i++)
            {
                info = property.Infos[i];
                stringBuilder.Append(property.Infos[i].Name)
                    .Append(Utility.ConvertValueType(info.GetValue(InValue, null), info.PropertyType))
                    .Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
                                           ...
                                           string[] InConnectOperators = null)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                stringBuilder.Append(InConditionFieldName[i])
                        ...
                        .Append(InOperator[i])
                        .Append(" ")
                        .Append(Utility.ConvertValueType(InConditionValue[i]));
                if (i < connectLength)
                    stringBuilder.Append(" ")
            ...
            string InConditionValue)
        {
            ...
            stringBuilder.Append("SELECT ")
                         ...
                         .Append(" = ")
                         .Append(Utility.ConvertValueType(InConditionValue));
                ...
                if (type.IsValueType)
                {
                    ...
                    else if (ConstData.BOOL_TYPE == type)
                    {
                        switch (SQLite3.ColumnType(InStmt, i))
                        {
                            ...
                            case SQLite3DataType.Text:
                                InPropertyInfos[i].SetValue(InBaseSubclassObj, SQLite3.ColumnText(InStmt, i).ToLower() == "true", null);
                                break;
                    ...
                    else if (ConstData.SHORT_TYPE == type)
                    {
                        InPropertyInfos[i].SetValue(InBaseSubclassObj, (short)SQLite3.ColumnInt(InStmt, i), null);
                    }
                    else if (ConstData.LONG_TYPE == type)
                    {
                        InPropertyInfos[i].SetValue(InBaseSubclassObj, SQLite3.ColumnInt64(InStmt, i), null);
                    }
                    else if (ConstData.DOUBLE_TYPE == type)
                    {
                        InPropertyInfos[i].SetValue(InBaseSubclassObj, SQLite3.ColumnDouble(InStmt, i), null);
                    }
                    else Debug.LogError("NOT SUPPORT TYPE : " + type);
                }
                else if(type.IsArray)
                {
                    InPropertyInfos[i].SetValue(InBaseSubclassObj, Utility.ConvertStringToArray(SQLite3.ColumnText(InStmt, i), type), null);
                }
                else if(ConstData.STRING_TYPE == type)
                {
